...
## 6. Run the armed loop (still no send)

```
pnpm soulmaker live:sniper:run --mode armed_canary --escalation-armed \
  --mint <MINT> --risk <MINT>=runs/risk.json --quote <MINT>=runs/quote-facts.json \
  --spend-sol 0.005 --json --out runs/run-report.json
```

If — and only if — every gate is green (live candidate, no risk block, fresh quote, policy green,
escalation permits one canary), the report's action is `prepare_canary_request`. Otherwise it tells
you exactly why it refused. **This command still sends nothing.**
