...
export const SIMULATION_REASON_CODES = [
  ...
  // --- diff (Sprint 73: structured-field artifact comparisons; append-only) ---
  "simulation-diff-plan-newly-blocked",
  "simulation-diff-plan-no-longer-blocked",
  "simulation-diff-plan-blocking-codes-changed",
  "simulation-diff-plan-warning-codes-changed",
  "simulation-diff-plan-source-refs-changed",
  "simulation-diff-plan-readiness-changed",
  "simulation-diff-plan-spec-adoption-changed",
  "simulation-diff-plan-acknowledgment-changed",
  "simulation-diff-plan-entries-changed",
  "simulation-diff-plan-identical",
  "simulation-diff-result-newly-blocked",
  "simulation-diff-result-no-longer-blocked",
  "simulation-diff-result-status-changed",
  "simulation-diff-result-blocking-codes-changed",
  "simulation-diff-result-adapter-changed",
...
> = {
  ...
  "simulation-diff-plan-newly-blocked": def("simulation-diff-plan-newly-blocked", "diff", "warning",
    "The next intent plan is BLOCKED where the base plan was not — something in the chain regressed; inspect the next plan's blocking reasons."),
  "simulation-diff-plan-no-longer-blocked": def("simulation-diff-plan-no-longer-blocked", "diff", "info",
    "The next intent plan is no longer blocked — the base plan's blocking reasons were resolved (still SIMULATION ONLY; never a live readiness signal)."),
  "simulation-diff-plan-blocking-codes-changed": def("simulation-diff-plan-blocking-codes-changed", "diff", "warning",
    "The plan-level blocking reason codes differ between base and next — inspect the added/removed codes before trusting either plan."),
  "simulation-diff-plan-warning-codes-changed": def("simulation-diff-plan-warning-codes-changed", "diff", "warning",
    "The plan-level warning reason codes differ between base and next — inspect the added/removed codes (e.g. an operator acknowledgment appearing or disappearing)."),
  "simulation-diff-plan-source-refs-changed": def("simulation-diff-plan-source-refs-changed", "diff", "warning",
    "The two plans disagree on at least one source artifact reference (presence/validity/schema/label) — they were not built from the same chain inputs."),
  "simulation-diff-plan-readiness-changed": def("simulation-diff-plan-readiness-changed", "diff", "warning",
    "The readiness summary (gates ready / prereqs met / their counts) differs between base and next — the underlying chain state moved."),
  "simulation-diff-plan-spec-adoption-changed": def("simulation-diff-plan-spec-adoption-changed", "diff", "warning",
    "The governance-spec adoption summary (kill-switch / secrets policy / burner isolation / stop-switch state) differs between base and next."),
  "simulation-diff-plan-acknowledgment-changed": def("simulation-diff-plan-acknowledgment-changed", "diff", "warning",
    "The applied paper-enter-review operator acknowledgment differs between base and next — an explicit human decision appeared or disappeared; review it."),
  "simulation-diff-plan-entries-changed": def("simulation-diff-plan-entries-changed", "diff", "warning",
    "The preview entries differ between base and next (membership, preview status, resolved/unresolved fields, or per-entry reason codes) — inspect the per-entry changes."),
  "simulation-diff-plan-identical": def("simulation-diff-plan-identical", "diff", "info",
    "The two intent plans are identical across every compared structured field — no change to review (side labels are metadata and are not compared)."),
  "simulation-diff-result-newly-blocked": def("simulation-diff-result-newly-blocked", "diff", "warning",
    "The next simulation result is BLOCKED where the base result was not — something in the chain regressed; inspect the next result's blocking reasons."),
  "simulation-diff-result-no-longer-blocked": def("simulation-diff-result-no-longer-blocked", "diff", "info",
    "The next simulation result is no longer blocked — the base result's blocking reasons were resolved (still SIMULATION ONLY; never a live readiness signal)."),
  "simulation-diff-result-status-changed": def("simulation-diff-result-status-changed", "diff", "warning",
    "The result-level status differs between base and next (e.g. skipped_unresolved → dry_run_unavailable) — inspect what moved before trusting either result."),
  "simulation-diff-result-blocking-codes-changed": def("simulation-diff-result-blocking-codes-changed", "diff", "warning",
    "The result-level blocking reason codes differ between base and next — inspect the added/removed codes."),
  "simulation-diff-result-adapter-changed": def("simulation-diff-result-adapter-changed", "diff", "warning",
    "The two results were produced by different dry-run adapters — their entry outcomes are not directly comparable; verify the adapter change was intended."),
  "simulation-diff-result-plan-ref-changed": def("simulation-diff-result-plan-ref-changed", "diff", "warning",
    "The two results reference different source plans (label/operator/blocked state/entry count) — they were not built from the same intent plan."),
  "simulation-diff-result-entries-changed": def("simulation-diff-result-entries-changed", "diff", "warning",
