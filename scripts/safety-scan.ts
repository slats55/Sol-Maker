...
/**
 * Sprint 107 — NARROW allowlist for the base58-blob rule ONLY.
 *
 * The committed live canary EXAMPLE artifacts embed an UNSIGNED transaction's base64
 * (`txpreview.envelope.v1` — every signature slot is exactly zero; this is PUBLIC transaction data,
 * never a key). The probe transaction's repetitive bytes produce a long base58-safe run that trips
 * the base58 secret-blob heuristic. These EXACT generated files (and nothing else) are exempt from
 * the base58-blob rule; every OTHER rule (keypair byte arrays, secret assignments, secret-bearing
 * filenames, .gitignore coverage) still applies to them in full. Regenerated by
 * `scripts/gen-live-examples.ts`. Do NOT widen this list — it is the only place the scan is relaxed,
 * and it is relaxed for one specific, reviewed, public-data shape.
 */
export const UNSIGNED_TX_EXAMPLE_BASE58_EXEMPT: ReadonlySet<string> = new Set([
...
export function scanContentForSecrets(text: string, fileName: string): SafetyFinding[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    //    The unsigned-tx example artifacts (PUBLIC, all-zero-signature transaction blobs) are the
    //    one narrow exemption — see UNSIGNED_TX_EXAMPLE_BASE58_EXEMPT.
    const base58Exempt = UNSIGNED_TX_EXAMPLE_BASE58_EXEMPT.has(fileName.replace(/\\/g, "/"));
    if (!isLock && !base58Exempt) {
